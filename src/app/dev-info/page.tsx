  ...
  return (
    ...
    >
      ...
      <p style={{ marginBottom: '30px', color: '#666' }}>
        ISR status, SEO tools, and development debugging information
      </p>
      ...
      >
        <h2>🔍 Debug Status</h2>
        <pre style={{ backgroundColor: 'transparent', color: 'inherit' }}>
          {JSON.stringify(debugInfo, null, 2)}
        </pre>
        <p>
          <strong>Content Load Time:</strong> {loadTime}ms
        </p>
        <p>
          <strong>Page Generated:</strong> {pageGenerated}
        </p>
      </div>
      ...
      >
        ...
        <ul>
          <li>
            <strong>This page:</strong> Client-side rendered (no ISR due to
            interactive features)
          </li>
          <li>
            <strong>Blog list page:</strong> Dynamic (shows new posts
            immediately)
          </li>
          <li>
            <strong>Blog posts revalidate every:</strong>{' '}
            {ISR_CONFIG.formatDuration(ISR_CONFIG.BLOG_POST_REVALIDATE)}
          </li>
        </ul>
        <p>
          <small>
            💡 <strong>Configure timing in:</strong>{' '}
            <code>src/config/isr.ts</code>
          </small>
        </p>
      </div>
      ...
      >
        ...
        <p>
          <strong>Count:</strong> {posts.length}
        </p>
        <p>
          <strong>Load Time:</strong> {loadTime}ms{' '}
          {loadTime > 1000
            ? '(Slow - likely fresh fetch)'
            : '(Fast - likely cached)'}
        </p>
        {posts.map((post, index) => {
          ...
          return (
            ...
            >
              <strong>
                {index + 1}. {String(typedPost.fields.title || 'Untitled')}
              </strong>
              <br />
              <small>
                ID: {typedPost.sys.id} | Updated: {typedPost.sys.updatedAt}
              </small>
            </div>
          )
        })}
      ...
      >
        ...
        <ol>
          <li>
            <strong>Refresh this page multiple times</strong> - Load time should
            vary
          </li>
          <li>
            <strong>Check browser Network tab</strong> - Look for Cache-Control
            headers
          </li>
          <li>
            <strong>Visit /blog</strong> - Dynamic page (shows new posts
            immediately)
          </li>
          <li>
            <strong>Update content in Contentful</strong> - Blog list shows
            changes immediately, individual posts update within{' '}
            {ISR_CONFIG.formatDuration(ISR_CONFIG.BLOG_POST_REVALIDATE)}
          </li>
        </ol>
      </div>
      ...
      >
        ...
        <ul>
          ...
          <li>
            ...
            - Includes all blog posts from Contentful
          </li>
          <li>
            <strong>
              <a href="/sitemap-isr.xml" target="_blank">
                /sitemap-isr.xml
              </a>
            </strong>{' '}
            - ISR-optimized sitemap (6h revalidation)
          </li>
        </ul>
        <p>
          <small>
            💡 <strong>Configure in:</strong> <code>src/config/site.ts</code>
          </small>
        </p>
      </div>
      ...
      >
        ...
        <p>
          Comprehensive documentation and testing tools for all available
          endpoints:
        </p>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          >
            <h4 style={{ margin: '5px 0', fontSize: '16px' }}>
              🔍 /api/status
            </h4>
            <p style={{ margin: '5px 0', fontSize: '14px', color: '#888' }}>
              Real-time server status, ISR configuration, environment info, and
              Contentful connectivity
            </p>
            <p style={{ margin: '5px 0', fontSize: '13px' }}>
              <strong>Returns:</strong> JSON with timestamp, ISR config,
              environment variables, cache headers
            </p>
            <p style={{ margin: '5px 0', fontSize: '13px' }}>
              <strong>Cache:</strong> No cache (always fresh data for
              monitoring)
            </p>
                ...
                onClick={() => {
                  fetch('/api/status')
                    ...
                    .then((data) => {
                      ...
                      pre.style.cssText =
                        'background: #1a1a1a; color: #00ff00; padding: 12px; border-radius: 4px; overflow-x: auto; margin: 10px 0; font-size: 11px; border: 1px solid #333;'
                      pre.textContent = JSON.stringify(data, null, 2)
                      const container = document.getElementById('api-results')
                      if (container) {
                        container.innerHTML =
                          '<h4 style="margin: 10px 0 5px 0; color: #3b82f6;">📊 /api/status Response:</h4>'
                    ...
                    .catch((err) => {
                      ...
                      if (container) {
                        container.innerHTML =
                          '<div style="background: #dc2626; color: white; padding: 10px; border-radius: 4px; margin: 10px 0;">❌ Error: ' +
                          err.message +
                          '</div>'
                      }
                    })
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          >
            ...
            <p style={{ margin: '5px 0', fontSize: '13px' }}>
              <strong>Content:</strong> Allow all crawlers, disallow /dev-info,
              references dynamic sitemap
            </p>
            <p style={{ margin: '5px 0', fontSize: '13px' }}>
              <strong>Cache:</strong> 24 hours (s-maxage=86400) for cost
              efficiency
            </p>
            <p style={{ margin: '5px 0', fontSize: '13px' }}>
              <strong>Config:</strong> <code>src/config/site.ts</code> →
              ROBOTS_CONFIG
            </p>
            <button
          ...
          >
            ...
            <p style={{ margin: '5px 0', fontSize: '14px', color: '#888' }}>
              Dynamic sitemap including all static pages + live blog posts from
              Contentful
            </p>
            <p style={{ margin: '5px 0', fontSize: '13px' }}>
              <strong>Content:</strong> Homepage, About, Blog index, all blog
              posts with proper lastmod dates
            </p>
            <p style={{ margin: '5px 0', fontSize: '13px' }}>
              <strong>Cache:</strong> 6 hours (s-maxage=21600) for balance of
              freshness vs. cost
            </p>
            <p style={{ margin: '5px 0', fontSize: '13px' }}>
              <strong>Config:</strong> <code>src/config/site.ts</code> →
          ...
          >
            ...
            <p style={{ margin: '5px 0', fontSize: '13px' }}>
              <strong>Content:</strong> Same as /sitemap.xml but with ISR
              revalidation
            </p>
            <p style={{ margin: '5px 0', fontSize: '13px' }}>
              <strong>Cache:</strong> 6 hours ISR revalidation (even more
              cost-effective)
            </p>
            <p style={{ margin: '5px 0', fontSize: '13px' }}>
              <strong>Use case:</strong> Consider switching production sitemap
              to this for maximum cost savings
            </p>
            <button
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          >
            🛠️ Developer Tools & cURL Commands
          </h3>
          ...
          >
            ...
            <div style={{ marginBottom: '8px' }}>
              <strong style={{ color: '#f59e0b' }}>
                Validate Sitemap (with cache info):
              </strong>
              <br />
              <code style={{ color: '#10b981' }}>
            ...
            <div style={{ marginBottom: '8px' }}>
              <strong style={{ color: '#ef4444' }}>
                Compare ISR Sitemap Performance:
              </strong>
              <br />
              <code style={{ color: '#10b981' }}>
                time curl -s{' '}
            ...
            <div>
              <strong style={{ color: '#6b7280' }}>
                Test All Endpoints (health check):
              </strong>
              <br />
              <code style={{ color: '#10b981' }}>
                for endpoint in api/status robots.txt sitemap.xml
                sitemap-isr.xml; do echo &quot;Testing /$endpoint:&quot;; curl
                -s -o /dev/null -w &quot;http_code: %{'{http_code}'}, time: %
                {'{time_total}'}s\n&quot;{' '}
                {typeof window !== 'undefined'
                  ? window.location.origin
                  : 'https://reggiepangilinan.com'}
          ...
          >
            ...
            >
              ...
              <button
                onClick={() => {
                  Promise.all([
                    fetch('/robots.txt').then((r) => ({
                      url: '/robots.txt',
                      status: r.status,
                      headers: Object.fromEntries(r.headers.entries()),
                    })),
                    fetch('/sitemap.xml').then((r) => ({
                      url: '/sitemap.xml',
                      status: r.status,
                      headers: Object.fromEntries(r.headers.entries()),
                    })),
                    fetch('/sitemap-isr.xml').then((r) => ({
                      url: '/sitemap-isr.xml',
                      status: r.status,
                      headers: Object.fromEntries(r.headers.entries()),
                    })),
        ...
        >
          ...
          >
            <li>
              Check browser Network tab (F12) for detailed cache headers and
              response times
            </li>
            <li>
              Use browser DevTools Performance tab to analyze page load timing
            </li>
            <li>Monitor Netlify Analytics for real-world performance data</li>
            <li>
              Test endpoints with different user agents to verify robots.txt
              compliance
            </li>
      ...
      >
        <h2>📊 Logging Info</h2>
        <p>
          Logs are written to console.error() for better visibility on Netlify.
        </p>
        <p>Check:</p>
        <ul>
        ...
        <p>
          <strong>Note:</strong> Netlify free tier has limited log retention.
          See NETLIFY_LOGGING_GUIDE.md for details.
        </p>
      </div>
      ...
      >
        ...
        <div style={{ marginBottom: '15px' }}>
          ...
          <p>
            ...
            {process.env.REVALIDATION_SECRET
              ? '✅ Configured'
              : '❌ Not configured (missing REVALIDATION_SECRET)'}
          </p>
          <p>
        ...
        <div style={{ marginBottom: '15px' }}>
          ...
          <ol>
            <li>
              <strong>Test endpoint status:</strong>{' '}
              <code>GET /api/revalidate?secret=YOUR_SECRET</code>
            </li>
            <li>
              <strong>Revalidate all blog content:</strong> Update any blog post
              in Contentful
            </li>
            <li>
              <strong>Manual revalidation:</strong>{' '}
              <code>POST /api/revalidate</code> with proper payload
            </li>
            <li>
              <strong>Check this page logs:</strong> Refresh to see if
              revalidation events are logged
            </li>
          </ol>
        </div>
  ...
  "slug": "your-blog-post-slug"
        ...
        <p>
          <small>
            ...
            <code>docs/guides/CONTENTFUL_WEBHOOK_GUIDE.md</code>
          </small>
        </p>
      </div>
...
REVALIDATION_SECRET=your-secret node scripts/test-revalidation.js`}
          ...
          <p style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
            <strong>📝 Note:</strong> Replace &quot;your-secret&quot; with your
            actual REVALIDATION_SECRET value.
            <br />
